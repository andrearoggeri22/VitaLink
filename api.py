...
from app import db
from models import Patient, VitalSignType, Note, VitalObservation
...
def add_vital(doctor, patient_uuid):
    ...
    # Check if the patient has a health platform connection
    if not patient.fitbit_access_token:
        return jsonify({"error": _("Patient does not have a health platform connection. Please connect first.")}), 400
    
    # This endpoint is deprecated - with new architecture, data comes from health platforms
    return jsonify({
        "error": _("This endpoint is deprecated. Vital signs are now retrieved from health platforms directly."),
        "message": _("Please refer to documentation for the new workflow using health platform integrations.")
    }), 410  # HTTP 410 Gone
