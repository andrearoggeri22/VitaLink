...
def check_device_connected():
    ...
    try:
        ...
        logging.info(
            _("Fitbit device connection check - simulation mode enabled"))
        return False
...
def extract_fitbit_data(patient_id):
    ...
    try:
        ...
        for i in range(7, 0, -1):
            ...
            distance = result_data['steps'][
                7 - i - 1]['value'] / 1300  # ~1300 steps per km
...
@fitbit_bp.route('/patients/<int:patient_id>/upload_fitbit',
                 methods=['GET', 'POST'])
@login_required
...
def mobile_upload_data():
    ...
    try:
        ...
        success, vitals_saved, errors = save_fitbit_data(
            patient_id, fitbit_data)

        # Log the mobile data upload action
        log_action(
            doctor_id=
            None,  # No doctor involved, it's an automatic action from mobile
